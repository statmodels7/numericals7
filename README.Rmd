...
past $10^5$ or lose large orders entirely. Following Plesner, Sørensen and
Hauberg (ICS 2024, arXiv:2409.08729), `log_bessel_i()` and
`log_bessel_k()` carry every intermediate quantity on the log scale, so
the result is finite and accurate wherever $\log I_\nu(x)$ itself is
representable; the `_derivs` variants add four derivatives in the argument
from the ratio identity and the Bessel equation, at the cost of one more
...
lesson learned there: the Mills ratio is formed on the log scale, where the
direct quotient is 0/0 from `t = -38` on; Owen's T batches every element
into one `quad_vec()` call; the Bessel ratio goes through the exponentially
scaled Bessel functions, so it is finite at any argument, and its inverse
comes with the four derivatives the inverse function rule provides.
